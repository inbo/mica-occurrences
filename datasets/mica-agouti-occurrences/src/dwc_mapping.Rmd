...
```{r}
occurrence %<>% mutate(dwc_datasetID = "") # NEW TERM
```
...
occurrence %<>% mutate(dwc_taxonRank = case_when(
  ...
  scientificName == "Canis lupus familiaris" ~ "subspecies", # Reassigned wolfs
  scientificName %in% c("Anas", "Anser") ~ "genus",
  scientificName %in% c("Columbidae", "Soricidae") ~ "family",
