...
subtitle: "MICA - Muskrat occurrences collected in East Flanders, Belgium"
...
```{r}
csv_files <- list.files(here::here("datasets", "mica-oostvlaanderen-occurrences", "data", "raw"), pattern = "*.txt$", full.names = TRUE)
...
input_interim <- read_delim(file = here::here("datasets", "mica-oostvlaanderen-occurrences", "data", "raw", "niet-rato_oostvl.txt" ), ";")
...
```{r}
occurrence %<>% mutate(datasetID = "https://doi.org/10.15468/****")
```
...
```{r}
occurrence %<>% mutate(rightsHolder = "East Flanders") # e.g. "INBO"
```
### accessRights
...
```{r}
occurrence %<>% mutate(institutionCode = "Province of East Flanders") # e.g. "INBO"
```

### datasetName

```{r}
occurrence %<>% mutate(datasetName = "MICA - Muskrat occurrences collected in East Flanders, Belgium") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
occurrence %<>% rename(occurrenceID = 'id') %>%
                mutate(occurrenceID = str_c("MICA:OVL.:", occurrenceID, ":", ID_row))
...
```{r}
occurrence %<>% mutate(recordedBy = 'Province of East Flanders')
```
