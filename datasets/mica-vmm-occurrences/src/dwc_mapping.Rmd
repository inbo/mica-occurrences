...
```{r}
occurrence %<>% select(-c(`Locatie Type Naam`, `Maand In Jaar`, `Maand Lange Omschrijving`, `Provincie NIS Code`))
```
...
```{r}
occurrence %<>% mutate(recordedBy = str_c("VMM:team:", `Team Naam`))
...
```{r}
occurrence %<>% mutate(samplingProtocol = 'rat_trap') 
```
...
```{r}
occurrence %<>%
  rename(decimalLongitude = `Locatie GPS Lengte`) %>%
  rename(decimalLatitude = `Locatie GPS Breedte`) %>%
  mutate(geodeticDatum = "WGS84") %>%
  mutate(coordinateUncertaintyInMeters = "30") %>%
  mutate(countryCode = "BE")  %>%            
  mutate(continent = "Europe") %>%
  mutate(locationID = `Locatie ID`) %>%
  mutate(locationRemarks = `VHA Categorie Omschrijving`)
...
```{r}
occurrence %<>% mutate(stateProvince = `Provincie Omschrijving`) # e.g. "Belgium = BE"
```
...
```{r}
occurrence %<>%
  mutate(municipality = `Gemeente Naam`)
```
...
occurrence %<>% mutate(locationRemarks = recode(locationRemarks
                   ...
                   , "CAT3 - Onbevaarbaar cat. 3" = "CAT3 - not naviable cat. 3"
...
occurrence %<>% select(-c(Jaar, Maand, Land_Regio, `Gemeente Naam`, `Locatie ID`, `Team Rol`, `Team Naam`,`Sporen Waarnemingen Naam`,`VHA Categorie Code`, `VHA Categorie Omschrijving`, `Provincie Omschrijving`))  #remove collection columns
...
col_order <- c( "type","language","license","rightsHolder","accessRights","datasetID"
               ...
               ,"identifiedBy","scientificName","vernacularName" ,"kingdom" ,"taxonRank","nomenclaturalCode"
               )
