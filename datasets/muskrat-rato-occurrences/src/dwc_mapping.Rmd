...
```{r}
input_interim <- read_csv(here::here("datasets", "muskrat-rato-occurrences", "data", "interim", "RATO_data_RAW_Refine.csv"))
```
...
write_csv(x_y, here::here("datasets", "muskrat-rato-occurrences", "data", "interim", "x_y.csv"), na = "")
```
...
```{r}
occurrence %<>% mutate(datasetID = "https://doi.org/10.15468/5fps96") # e.g. "https://doi.org/10.15468/xvuzfh"
```
...
head(occurrence, n = 5)
occurrence %<>%
mutate(eventDate = as_date(eventDate, format = "%Y-%m-%d")) 
```
...
```{r}
write_csv(occurrence, here::here("datasets", "muskrat-rato-occurrences", "data", "processed", " occurrence.csv"), na = "")
```
